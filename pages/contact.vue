...
async function onSubmit(event: FormSubmitEvent<Schema>) {
  ...
  const data = await $fetch(url, {
    ...
    body: {
      ...
      nonce,
      sign: md5(JSON.stringify(event.data) + (new Date().getTime() / 1000).toFixed(0) + nonce + key) // 这个sign是用md5加密的，加密规则 md5(JSON.stringify(data) + timestamp + nonce + 'v0Wxg11Jl6hH2qxLf8')
    },
    headers: {
